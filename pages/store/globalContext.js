...
export function GlobalContextProvider(props) {
  ...
  async function editGlobalData(command) {
    ...
    if (command.cmd == "deleteMeeting") { //  {cmd: 'deleteMeeting', newVal: meetingId}
   // console.log('command.newVal:' + command.newVal)
   // return
      const newMeetingsArray = []
      for (let ii = 0; ii <globals.meetings.length; ii++) {
          if(globals.meetings[ii].meetingId != command.newVal) {
            newMeetingsArray.push(globals.meetings[ii])
          }
      }
      globals.meetings = newMeetingsArray
      const response = await fetch("/api/save-meetups", {
        method: "POST",
        body: JSON.stringify(globals.meetings),
        headers: {
          "Content-Type": "application/json",
        },
      });
